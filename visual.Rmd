...
```{r neb_plot, fig.width = 8, fig.height = 6, echo = FALSE}
neb_plot = hiv_data %>% 
  group_by(uhf, gender) %>% 
...
```{r age_plot, fig.width = 8, fig.height = 6, echo = FALSE}
age_plot = hiv_data %>% 
...
```{r HIV-related-death-rate, fig.width = 9.6, fig.height = 6, echo = FALSE}
...
```{r overall_year, fig.width = 8, fig.height = 6, echo = FALSE}
...
```{r HIV-income, fig.width = 9.6, fig.height = 6, echo = FALSE}
...
```
**Interpretation**:
the correlation between `hiv_diagnoses` and `mid_income` is `r cor(dis$hiv_diagnoses, dis$mid_income)`, which indicates overall HIV incidence decreases as the overall median income increases.
